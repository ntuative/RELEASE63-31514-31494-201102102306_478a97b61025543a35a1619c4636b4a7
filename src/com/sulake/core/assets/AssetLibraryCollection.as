package com.sulake.core.assets
{
   ...
   public class AssetLibraryCollection extends EventDispatcher implements IAssetLibrary
   {
      ...
      public function AssetLibraryCollection(param1:String)
      {
         ...
         this.var_993 = new LoaderContext(false,ApplicationDomain.currentDomain,null);
         this.var_438 = new AssetLibrary("lib-" + this.var_532++);
      ...
      public function loadFromFile(param1:LibraryLoader, param2:Boolean = false) : void
      {
         ...
         {
            this.loaderContext = this.var_993;
         }
         var _loc3_:IAssetLibrary = new AssetLibrary("lib-" + this.var_532++);
         this.var_530.push(_loc3_);
      ...
      {
         while(this.var_530.length > 0)
         {
            (this.var_530.pop() as IAssetLibrary).dispose();
         }
      ...
      private function loadEventHandler(param1:LibraryLoaderEvent) : void
      {
         ...
         if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE)
         {
            ...
            _loc4_ = 0;
            while(_loc4_ < this.var_530.length)
            {
               _loc3_ = this.var_530[_loc4_] as IAssetLibrary;
               if(_loc3_.url == _loc2_.url)
               {
                  this.var_530.splice(_loc4_,1);
                  break;
               }
               _loc4_++;
            }
