package com.sulake.core.runtime.events
{
   ...
   public class EventDispatcher implements IEventDispatcher, IDisposable
   {
      ...
      private static const const_1260:uint = 1;
      
      private static const const_920:uint = 2;
      ...
      public function addEventListener(param1:String, param2:Function, param3:Boolean = false, param4:int = 0, param5:Boolean = false) : void
      {
         ...
         else
         {
            ...
            {
               if(_loc8_.callback == param2 && _loc8_.var_1943 == param3)
               {
                  return;
               }
      ...
      public function removeEventListener(param1:String, param2:Function, param3:Boolean = false) : void
      {
         ...
         if(!this._disposed)
         {
            ...
            if(_loc4_)
            {
               ...
               {
                  if(_loc6_.callback == param2 && _loc6_.var_1943 == param3)
                  {
      ...
      public function dispatchEvent(param1:Event) : Boolean
      {
         if(!this._disposed)
         {
            this._result = const_588;
            this._eventDispatcher.dispatchEvent(param1);
            if(this._result == const_920)
            {
               if(this.var_577 != null)
               {
                  Core.crash("Error catched when handling " + getQualifiedClassName(param1) + ": " + this.var_577.message,this.var_577.errorID,this.var_577);
               }
               else
               {
                  Core.crash("Error catched when handling " + getQualifiedClassName(param1) + ". No error data available!",0,this.var_577);
               }
            }
            return this._result == const_588;
         }
         return false;
      }
      ...
      private function eventProcessor(param1:Event) : void
      {
         ...
         this._result = !!event.isDefaultPrevented() ? uint(const_1260) : uint(const_588);
      }
