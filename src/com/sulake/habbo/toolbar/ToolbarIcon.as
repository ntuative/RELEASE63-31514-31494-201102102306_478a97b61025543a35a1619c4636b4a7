package com.sulake.habbo.toolbar
{
   ...
   public class ToolbarIcon
   {
      ...
      private var var_633:ToolbarIconAnimator;
      ...
      public function ToolbarIcon(param1:ToolbarIconGroup, param2:IHabboWindowManager, param3:IAssetLibrary, param4:String, param5:IEventDispatcher, param6:ToolbarBarMenuAnimator)
      {
         ...
         this.var_1183 = new ToolbarIconBouncer(0.8,1);
         super();
         this.var_517 = param1;
      ...
      public function set exists(param1:Boolean) : void
      {
         ...
         if(this._region != null)
         {
            this._region.visible = this.var_1184;
         }
      }
      ...
      public function defineFromXML(param1:XML) : void
      {
         ...
         this.var_1441 = param1.@id;
         this.var_2661 = int(param1.@window_offset_from_icon);
         this.var_2512 = int(param1.@window_margin);
         this.var_2771 = param1.@label;
         ...
         if(_loc2_.length() > 0)
         {
            ...
            {
               this.var_1181 = param1.@state_default;
            }
            _loc3_ = 0;
            while(_loc3_ < _loc2_.length())
            {
               ...
               this.var_1185.push(_loc5_.id);
               this.var_1883.push(_loc5_);
               if(_loc3_ == 0)
               {
                  if(this.var_1442 == "-1")
                  {
                     this.var_1442 = _loc5_.id;
                  }
                  if(this.var_1181 == "-1")
                  {
                     this.var_1181 = _loc5_.id;
                  }
               }
               _loc3_++;
            }
         }
         this._state = this.var_1181;
      }
      ...
      private function setAnimator() : void
      {
         ...
         if(_loc1_ != null && this.var_1184)
         {
            if(_loc1_.frames == null)
            {
               return;
            }
            this.var_633 = new ToolbarIconAnimator(_loc1_,this._assetLibrary,this._icon,this._x,this.var_166,this._bitmapData);
            if(this.var_633.frameCount > 0)
            {
               this.var_953 = new Timer(_loc1_.timer);
               this.var_953.addEventListener(TimerEvent.TIMER,this.updateAnimator);
               this.var_953.start();
            }
            if(_loc1_.bounce)
            {
               this.var_1183.reset(-7);
               this.var_1443.reset();
               this.var_1443.start();
            }
         }
      ...
      {
         var _loc4_:* = null;
         if(this.var_774.indexOf(param1) < 0)
         {
            this.var_774.push(param1);
            _loc4_ = new MenuSettingsItem(param1,param2,param3);
            this.var_1182.push(_loc4_);
         }
      }
      
      public function docksMenu(param1:String) : Boolean
      {
         return this.var_774.indexOf(param1) > -1;
      }
      ...
      public function menuLockedToIcon(param1:String) : Boolean
      {
         ...
         var _loc2_:MenuSettingsItem = this.var_1182[this.var_774.indexOf(param1)];
      ...
      {
         return this.var_1883[this.var_1185.indexOf(param1)];
      }
      ...
      private function setTooltip() : void
      {
         ...
         {
            this._region.toolTipCaption = "${toolbar.icon.tooltip." + this.var_1441.toLowerCase() + "}";
         }
      ...
      private function setLabel() : void
      {
         ...
         {
            this.var_365.caption = "${toolbar.icon.label." + this.var_1441.toLowerCase() + "}";
         }
         this.updateRegion();
      }
      ...
      {
         return this.var_2512 + this.var_517.windowMargin;
      }
      
      private function updateAnimator(param1:Event) : void
      {
         if(this.var_633 != null && this._icon != null)
         {
            this.var_633.update(this._icon);
            if(this.var_633.hasNextState())
            {
               this.state = this.var_633.nextState;
            }
         }
      }
      ...
      {
         if(this.var_1183 != null && this._icon != null)
         {
            this.var_1183.update();
            this._icon.y = this.var_1183.location;
         }
      }
         ...
         switch(param1.type)
         {
            case WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK:
               ...
               if(this._events != null)
               {
                  ...
                  _loc4_.iconName = this.var_1441;
                  this._events.dispatchEvent(_loc4_);
               }
               break;
            case WindowMouseEvent.WINDOW_EVENT_MOUSE_OVER:
               if(this.var_1185.length > 0)
               {
      ...
      private function updateRegion() : void
      {
         ...
         var _loc1_:Boolean = this.var_517.orientation == "LEFT" || this.var_517.orientation == "RIGHT";
         if(_loc1_)
         {
            this._x = (this.var_517.toolbarUsableWidth - this._region.width) / 2;
            this.var_166 = this.var_1057 + (this.var_517.iconSize - this._region.height) / 2;
         }
         else
         {
            this._x = this.var_1057 + (this.var_517.iconSize - this._region.width) / 2;
            this.var_166 = (this.var_517.toolbarUsableWidth - this._region.height) / 2;
         }
