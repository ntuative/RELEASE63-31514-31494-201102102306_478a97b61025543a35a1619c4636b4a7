package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_258:IRoomObjectSprite;
      ...
      private var var_1170:Number = 1.0;
      
      private var var_627:BitmapData;
      
      private var var_763:ColorTransform;
      
      private var var_1432:ColorTransform;
      
      private var var_1860:Matrix;
      
      private var _translationMatrix:Matrix;
      
      private var var_754:Number = 1.0;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_763 = new ColorTransform();
         this.var_763.alphaMultiplier = 1;
         this.var_1432 = new ColorTransform();
         this.var_1860 = new Matrix();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_93 = this.var_1171[param1];
         this.var_940 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1861 >= 0)
         {
            this.var_258 = this._visualization.getSprite(this.var_1861);
            if(this.var_258 && this.var_258.asset)
            {
               if(this.var_258.width <= 1 || this.var_258.height <= 1)
               {
                  return;
               }
               if(this.var_84 == null)
               {
                  this.var_84 = this.var_258.asset.clone();
                  if(this.var_763.alphaMultiplier != 1)
                  {
                     this.var_627 = new BitmapData(this.var_84.width,this.var_84.height,true,4278190080);
                  }
               }
               this.var_2638 = -this.var_258.offsetX;
               this.var_2639 = -this.var_258.offsetY;
               this.var_258.asset = this.var_84;
            }
            if(this.var_84)
            {
            ...
            if(this.var_627)
            {
               this.var_627.fillRect(this.var_627.rect,4278190080);
            }
         }
      }
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_93.y * this.var_1170;
         }
         return 0;
      }
      ...
      {
         if(!this.var_93 || !this.var_258)
         {
            return;
         }
         if(this.var_84 && this.var_258.asset != this.var_84)
         {
            this.var_258.asset = this.var_84;
         }
         if(this.var_940)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_940 && this.var_93.hasIgnited)
         {
            this.var_940 = true;
         }
         _loc8_ = this.var_1059 * this.var_1170;
         this.var_93.update();
         if(this.var_940)
         {
            if(this.var_93.roomObjectSpriteId >= 0)
            ...
            {
               this.var_84.draw(this.var_627,this.var_1860,this.var_763,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_93.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2638 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1170;
               _loc4_ = this.var_2639 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1170;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1432.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_84.draw(_loc7_,this._translationMatrix,this.var_1432,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2637 = parseInt(param1.@size);
         this.var_1861 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1059 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1170 = this.var_2637 / 64;
         this.var_754 = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this.var_754 = Math.min(this.var_754,1);
         this.var_763.alphaMultiplier = this.var_754;
         for each(_loc3_ in param1.emitter)
         {
            ...
            for each(_loc23_ in _loc3_.particles.particle)
            {
               ...
               _loc20_ = _loc23_.hasOwnProperty("@fade") && _loc23_.@fade == "HabboFriendListCom_opened_to_web_popup_xml" ? true : false;
               _loc21_ = [];
