package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.var_426 = this.getPetTypeIndexFromProduct(_loc5_.localizationId);
         if(this.var_426 >= 8)
         {
            return false;
         }
         ...
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc9_ = _loc8_.avatarRenderManager.petDataManager.getPetData(this.var_426,this);
               if(_loc9_)
               {
                  this.var_224 = _loc9_.sellableBreeds;
                  this.var_363 = _loc9_.colors;
         ...
         {
            _loc10_ = this.var_224[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(this.getRaceLocalizationKey(this.var_426,_loc10_.id),this.getRaceLocalizationKey(this.var_426,_loc10_.id)));
            _loc6_++;
         }
         this._offers.add(this.var_426,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_853,this.onApproveNameResult);
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         this.var_515 = param1.index;
         if(this.var_515 < 0 || this.var_515 > this.var_363.length)
         {
            this.var_515 = 0;
         }
         this.updateImage();
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_515 >= 0 && this.var_515 < this.var_363.length)
         {
            _loc7_ = this.var_363[this.var_515] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = this.var_224[this.var_1875] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_426,_loc3_.id,_loc2_,AvatarScaleType.const_53,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_309,AvatarAction.const_887);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_515 >= this.var_363.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_363[this.var_515] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = this.var_224[this.var_1875] as IBreed;
