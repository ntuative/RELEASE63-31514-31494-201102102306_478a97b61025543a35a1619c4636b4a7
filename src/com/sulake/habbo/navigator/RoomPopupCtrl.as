package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2276 = new RoomThumbnailRenderer(param1);
         this.var_1087 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_173 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_173.visible = true;
         Util.hideChildren(this.var_173);
         this.refreshRoomName(this.var_173,this.var_111);
         if(this.var_111.ownerName != "" && this.var_111.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_173,!!this.var_111.event ? this.getEventCtg(this.var_111.categoryId) : this.getRoomCtg(this.var_111.categoryId));
         this.refreshRoomDesc(this.var_173,this.var_111);
         this.refreshExtraCont();
         this.refreshInfo(this.var_173,"trading_allowed",this.var_111.allowTrading);
         this.refreshInfo(this.var_173,"doormode_doorbell",this.var_111.doorMode == 1);
         this.refreshInfo(this.var_173,"doormode_password",this.var_111.doorMode == 2);
         Util.moveChildrenToColumn(this.var_173,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_173.height = Util.getLowestPoint(this.var_173);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_173,this.var_111.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_173.findChildByName("roomowner"));
         if(this.var_111.ownerName != "" && this.var_111.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_173.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1087.refreshTags(_loc1_,this.var_111.tags);
         ...
         this.var_2276.refreshThumbnail(_loc1_,this.var_111.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_224,Tabs.const_218,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
