package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_851:int = 3;
      
      private var var_394:AssetLoaderStruct;
      ...
      private const const_1950:int = 100;
      
      private const const_1631:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_394 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_849 = new Timer(this.const_1950,1);
         this.var_849.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_394)
         {
            this.var_394.addEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
            this.var_394.addEventListener(AssetLoaderEvent.const_44,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_394)
         {
            this.var_394.removeEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
            this.var_394.removeEventListener(AssetLoaderEvent.const_44,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_851);
            if(this.var_1305.indexOf("?") > 0)
            {
               _loc2_ = this.var_1305 + "&retry=" + this.var_851;
            }
            else
            {
               _loc2_ = this.var_1305 + "?retry=" + this.var_851;
            ...
            this.var_394 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function loadFigureMapData(param1:XML) : void
      {
         ...
         this.var_1610 = true;
         for each(_loc2_ in this.var_1064)
         {
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2245,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1065[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1065[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1610)
         {
            this.var_1064.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_518[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_518[_loc3_] = _loc6_;
            }
            this.var_850[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_850)
         {
            _loc7_ = true;
            _loc4_ = this.var_850[_loc3_];
            ...
            if(_loc7_)
            {
               ...
               delete this.var_518[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_850[_loc3_];
         }
         while(_loc6_ < this.var_682.length)
         {
            _loc5_ = this.var_682[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_682.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_849.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_681.length > 0 && this.var_682.length < this.const_1631)
         {
            _loc1_ = this.var_681[0];
            _loc1_.startDownloading();
            this.var_682.push(this.var_681.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_681.indexOf(param1) == -1 && this.var_682.indexOf(param1) == -1)
         {
